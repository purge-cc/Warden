...
pub struct InitOverrides {
    /// Local Warden display name; defaults to the hostname.
...
pub fn run_init(
    ...
    append_node_identity(&mut body, overrides.node_name.as_deref())?;
...
fn run_init_cluster_secondary(
    ...
    let mut body = render_cluster_secondary_config(listen, &allow_from, &layout.socket_path, peer);
    append_node_identity(&mut body, overrides.node_name.as_deref())?;
...
fn render_cluster_secondary_config(
    ...
    out.push_str("# purge-warden configuration — CLUSTER SECONDARY (schema v5)\n");
    ...
    out.push_str(
        "#\n\
         ...
         # [[custom_lists]] or [[labels]] here: the loader would MERGE them with\n\
    ...
    writeln!(out, "\nschema_version = {TARGET_SCHEMA_VERSION_V5}").unwrap();
...
fn append_node_identity(body: &mut String, name: Option<&str>) -> anyhow::Result<()> {
    let mut node = crate::config::schema::NodeConfig {
        id: None,
        name: name.map(str::to_owned),
        control_listen: None,
    };
    node.ensure_identity().map_err(anyhow::Error::msg)?;
    body.push_str("\n[node]\n");
    body.push_str(&toml::to_string(&node)?);
    Ok(())
}
...
fn render_default_config(
    ...
    let mut out = String::new();
    out.push_str("# purge-warden configuration (schema v5)\n");
    ...
    out.push_str(
        "\n# Example [[devices]] / [[groups]] / [[subnets]] entries (commented out)\n\
         ...
         # device_type  = \"iPhone personale\"         # optional model/type description\n\
...
mod tests {
    use super::*;
    use crate::config::schema::{ConfigV1, ConfigV5};
    ...
    fn the_cluster_secondary_scaffold_carries_no_policy() {
        ...
        let cfg: ConfigV5 = toml::from_str(&body).expect("the scaffold parses as schema 5");
    ...
    fn default_config_is_valid_toml() {
        let body = default_config();
        let parsed: Result<ConfigV5, _> = toml::from_str(&body);
        assert!(
            parsed.is_ok(),
            "default config should be valid TOML: {parsed:?}"
        );
        let parsed = parsed.unwrap();
        assert_eq!(parsed.schema_version, TARGET_SCHEMA_VERSION_V5);
        assert!(
            !body.contains("admin_rules")
                && !body.contains("allow_rules")
                && !body.contains("deny_rules")
                && !body.contains("override_profile_deny")
                && !body.contains("tags =")
                && !body.contains("# device       ="),
            "schema-5 scaffold must not emit retired schema-4 fields: {body}"
        );
    }
    ...
    fn rendered_template_round_trips_through_validator_and_resolver() {
        ...
        use crate::config::schema::validator::AuditWarnings;
        use crate::config::target_v5::{
        ...
        let mut target: ConfigV5 =
            toml::from_str(&body).expect("rendered template parses as ConfigV5");
        assert_eq!(target.schema_version, TARGET_SCHEMA_VERSION_V5);
        ...
        let TargetV5Error::Validation(errs) = errs else {
            panic!("expected schema-5 validation errors, got: {errs}");
        };
        ...
        target.upstream.servers = vec!["192.0.2.53:53".to_string()];
        validate_v5_collect(
            &target,
            OffsetDateTime::now_utc(),
            &mut AuditWarnings::silent(),
        ...
        let cfg = target.validation_projection().unwrap();
        ...
        let list_bit_map =
            SourceBitMap::build(&merged_sources, &cfg.blocklists).expect("at-cap accept");
        let admission =
            CompileAdmission::new(target.custom_list_limits.max_compiled_bytes_total, 1).unwrap();
        let compiled = compile_v5_operator_rules(&target, &PackBodiesV5::default(), &admission)
            .expect("the scaffold operator policy compiles");
        let _resolver = ProfileResolver::build_with_operator_rules(&cfg, Arc::new(compiled));
