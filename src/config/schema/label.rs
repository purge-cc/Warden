...
//!   config on disk unloadable — `"Alex"` and `"Apple TV"` are not
...
pub struct Label {
    ...
    /// `owner = "Alex"` is inside the vocabulary if some `owner`
...
impl Label {
    ...
    /// the live configs hold `owner = "Alex"`, which can never equal
...
"#,
        ...
        .unwrap();
        assert_eq!(l.id.as_str(), "alex");
        assert_eq!(l.kind, LabelKind::Owner);
        assert_eq!(l.display_name, "Alex");
...
"#,
    ...
    fn invalid_id_rejected() {
        for bad in ["Alex", "-alex", "alex-", "edo ardo"] {
